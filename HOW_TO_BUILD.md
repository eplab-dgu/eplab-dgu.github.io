...
```
**확인**: 토글·갤러리 동작 정상, JS 없이도 콘텐츠는 보이게(점진적 향상).
...
- 인증 실패/네트워크 오류 시 직전 성공 데이터 또는 명확한 에러로 빌드가
  통째로 깨지지 않게 처리.
```
**확인**: 시트에서 실데이터 로드, `publish` 체크박스 토글이 다음 빌드에 반영.
...
- **콘텐츠 수정은 시트에서**: 코드가 아니라 `eplab_website_content.xlsx`(→ 나중에 Google Sheet)를 고쳐라. 그게 이 프로젝트의 목적.
