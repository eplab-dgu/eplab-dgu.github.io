...
| Contact 지도 | 사진이 아니라 **네이버 지도**입니다 — 아래 별도 항목 참고 |
...
### 그림 크기 (비율이 안 맞으면 잘립니다)

슬롯 비율과 그림 비율이 다르면 **넘치는 쪽이 잘려 나갑니다**(가운데를 기준으로 채웁니다).

| 어디 | 비율 | 권장 크기 |
|---|---|---|
| **연구주제**(`Research_Topics.image_url`) | **16:10** | **1600 × 1000 px** |
| 구성원 사진(`Members.photo_url`) | 1:1 (정사각) | 800 × 800 px — 증명사진(2:3)은 위쪽 기준으로 잘립니다 |
| Leader 사진 | 1:1 (정사각, 여백 허용) | 900 × 900 px — 세로 사진도 잘리지 않고 좌우에 여백이 생깁니다 |
| 갤러리(`Gallery.image_urls`) | 4:3 | 1200 × 900 px |

> 연구주제 그림은 **홈 화면 카드와 Research Topics 페이지 두 곳**에 같은 그림이 쓰입니다.
> 두 곳 모두 16:10 이므로 한 장만 맞춰 만들면 됩니다.
...
### Contact 페이지 지도 (구글 지도)

**따로 설정할 것이 없습니다.** 장소 이름으로 찾아 보여주므로 API 키도, 좌표도 필요 없습니다.
기본값은 `동국대학교 원흥관` 입니다.

바꾸고 싶으면 `Site_Config` 탭에 행을 추가하세요(둘 다 선택 사항):

| key | value_ko | 설명 |
|---|---|---|
| `map_query` | `동국대학교 원흥관` | 지도에서 찾을 장소. 주소를 넣어도 됩니다 |
| `map_zoom` | `17` | 확대 정도. 숫자가 클수록 확대 |

> **지도 확대·축소는 `Ctrl` + 마우스 휠**, 또는 더블클릭·지도 오른쪽 아래 `+`/`−` 버튼입니다.
> 그냥 휠을 굴리면 페이지가 스크롤됩니다 — 구글이 일부러 그렇게 해둔 것입니다.
> (페이지를 내리다 지도 위를 지날 때 지도가 확대돼 버리는 것을 막습니다.)
