...
### Contact 페이지 지도 (네이버 지도)

지도는 사진이 아니라 **네이버 지도를 실제로 띄우는 것**이라, 네이버에서 발급받은
**Client ID** 가 있어야 나옵니다. 없으면 "[ MAP ]" 안내와 네이버 지도 링크만 보입니다.

**Client ID 발급** (한 번만 하면 됩니다)

1. <https://www.ncloud.com> 가입 → 콘솔
2. **Services → Application Services → Maps** → 이용 신청
3. **Application 등록** → **Web Dynamic Map** 체크
4. **서비스 URL** 에 `https://eplab-dgu.github.io` 등록 ← 이걸 빠뜨리면 지도가 안 뜹니다
5. 발급된 **Client ID** 복사

**시트에 넣기** — `Site_Config` 탭에 행을 추가합니다.

| key | value_ko | 설명 |
|---|---|---|
| `naver_map_client_id` | (발급받은 Client ID) | **이게 있어야 지도가 나옵니다** |
| `map_lat` | `37.5582` | 지도 중심 위도 (선택) |
| `map_lng` | `127.0001` | 지도 중심 경도 (선택) |
| `map_zoom` | `17` | 확대 정도, 숫자가 클수록 확대 (선택) |

> **정확한 좌표 얻는 법**: 네이버 지도에서 원흥관 위치를 찾아 **마우스 오른쪽 클릭 →
> "이 위치의 좌표 복사"**. 나오는 두 숫자가 위도·경도입니다.
> 기본값은 캠퍼스 중심이라 건물과 조금 다를 수 있습니다.

Client ID 는 웹페이지에 그대로 노출되는 값입니다(원래 그런 종류의 키입니다).
대신 위 4번에서 등록한 주소에서만 동작하므로 다른 사이트에서 가져다 쓸 수 없습니다.
